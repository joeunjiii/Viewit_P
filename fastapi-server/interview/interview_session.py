...
class InterviewSession:
    ...
    def get_random_common_question(self) -> str | None:
        ...
        if results:
            questions = [r.payload.get("question") for r in results if r.payload.get("question")]
    ...
    def search_similar_questions(self, query: str, top_k: int = 5) -> list[tuple[str, float]]:
        query_vector = self.embedder.encode(query).tolist()
        filters = [{"key": "job_role", "match": {"value": self.job_role}}]
        if self.softskill_label:
            filters.append({"key": "softskill_label", "match": {"value": self.softskill_label}})
    ...
    def store_answer(self, question: str, answer: str, topic: str | None = None) -> None:
        self.state["history"].append({"question": question, "answer": answer, "topic": topic})
    ...
    def is_too_similar_to_previous(self, new_question: str, threshold: float = 0.9) -> bool:
