...
아래 조건을 반드시 지키세요:
- 공통 소프트스킬(협업, 의사소통, 갈등관리, 자기주도성, 성장경험, 배려심 등)에 관한 실제 면접 현장 느낌의 질문을 1~2문장 이내로 작성할 것
- 최근 3개 이내 질문/주제와 유사하거나 반복되는 질문은 절대 금지
- 답변이 짧거나 모호하면, 새로운 각도(예: 상황·사례 요구, 동료 관점 등)에서 추가 설명을 유도하는 꼬리질문 포함
- 사용자가 부담 없이 대답할 수 있도록 부드럽고 친근한 말투 사용
- 질문만 출력, 예시/설명/불필요한 문장 X
...
위의 [좋은 예]처럼, 자연스럽고 구체적인 면접 질문을 1개만 출력하세요. (질문만 출력)
"""
            for _ in range(3):
...
- 반드시 '{self.job_role}'와 직접적으로 관련된 실무/현장 중심의 질문을 포함할 것
- 최근 3개 이내 질문, 동일/유사 주제는 절대 반복 금지 (반드시 새로운 시각, 세부사항, 경험을 묻는 질문일 것)
- 지원자의 마지막 답변 내용({answer_desc})을 반드시 참고해서, 모호하면 추가 설명·사례·구체적 경험을 유도할 것
- 면접 분위기는 부드럽고 친근하게, 그러나 전문성은 명확하게
- 질문 길이는 1~2문장 이내
- 질문 예시를 생성하지 마세요. 반드시 실제 질문만 출력하세요.
- 지원자의 답변에 대해 더 깊이 파고드는 후속질문(꼬리질문, follow-up question)만 생성하세요.
...
위 [좋은 예] 스타일로, 위의 지침을 반드시 지켜 질문을 1개만 생성하세요.
질문만 출력하세요. (설명, 예시, 메타설명 절대 금지)
- """
        for _ in range(3):
            response = self.openai.chat.completions.create(
                model="gpt-4o",
