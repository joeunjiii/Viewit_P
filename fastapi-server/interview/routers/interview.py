...
async def next_question(
    ...
    try:
        ...
        feedback_service.save_answer_feedback_to_spring(
            data.session_id, last_q, answer_fb, SPRING_URL, token=authorization
        )
    except Exception as e:
        print(f"[WARN] 개별 피드백 저장 실패: {e}")

    # 3) 5분 경과 시 final 질문 제공
    if time.time() - session.start_time >= 300 and not session.state.get(
        ...
        interviewerVoice = session.state.get("interviewerVoice", "r2b2z8wPmZeh7CQksHSs")
        audio_url = generate_tts_audio(final_q, interviewerVoice)
    ...
    interviewerVoice = session.state.get("interviewerVoice", "r2b2z8wPmZeh7CQksHSs")
    try:
        audio_url = generate_tts_audio(next_q, interviewerVoice)
    except Exception:
