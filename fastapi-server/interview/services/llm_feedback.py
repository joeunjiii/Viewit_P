...
1. 전체 면접에 대한 총평 (3~4줄, 논리력, 적극성, 직무적합성 등 종합적으로 평가)
2. 강점 (공감능력, 이해심, 논리력, 적극성, 책임감 등, **키워드만 콤마로 구분해서 2~3개**)
3. 약점 (**키워드만 콤마로 구분해서 2~3개**)
...
[금지 예시]
강점: 지원자는 공감능력이 뛰어나며 책임감이 강하고 논리적으로 사고합니다. (이렇게 문장으로 쓰지 마세요)
약점: 때때로 세부적인 부분에 집중하다가 전체 흐름을 놓칠 수 있습니다. (이렇게 문장으로 쓰지 마세요)

강점/약점은 반드시 콤마(,)로 구분된 한글 키워드만 작성하세요!
    """
    resp = llm.chat.completions.create(
        model="gpt-4o",
        messages=[
